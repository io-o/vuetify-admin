  ...
  <div>
    ...
    >
        ...
        <v-toolbar flat color="white">
          ...
          <v-dialog v-model="dialog" fullscreen>
            <template v-slot:activator="{ on }">
              <div>
                <v-btn id="insert" v-permission="btnRole" color="primary" dark class="mb-2" v-on="on">新增受试者</v-btn>
      ...
      <template v-slot:item.name="{ item }">
        <v-btn text color="primary" @click="goDataCollect(item)">{{ item.centerName }}</v-btn>
      </template>
...
export default {
  ...
  data: () => ({
    ...
    length: 0,
    headers: [
      { text: '受试者姓名', align: 'left', sortable: false, value: 'name' },
      { text: '受试者姓名编号', value: 'centerNumber', sortable: false },
      { text: '创建日期', value: 'email', sortable: false },
  ...
  methods: {
    ...
    goDataCollect(item) {
      // 跳转数据采集页面
      this.$router.push({ path: `/subject/dataCollect?id=${item.centerId}` })
    }
  }
}
</script>
